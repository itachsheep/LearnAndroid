...
public class OfflineListAdapter extends BaseExpandableListAdapter implements
        OnGroupCollapseListener, OnGroupExpandListener {
	
	
	private boolean[] mIsOpen;// 记录一级目录是否打开
...
//		this.cityMap = cityMap;
		...
		mIsOpen = new boolean[provinceList.size()];
	}
...
//		return cityMap.get(groupPosition).get(childPosition).getCity();
		return provinceList.get(groupPosition).getCityList().get(childPosition);
	}
	...
	public View getChildView(int groupPosition,
		...
		if (convertView != null) {
			...
			offLineChild = new OfflineChild(mContext, amapManager);
...
//				childPosition);
		...
		offLineChild.setOffLineCity(mapCity);
...
//		return groupPosition != 0 && groupPosition != 1 && groupPosition != 2;
	...
	public void onGroupCollapse(int groupPosition) {
		mIsOpen[groupPosition] = false;
	}

	@Override
	public void onGroupExpand(int groupPosition) {
		mIsOpen[groupPosition] = true;
	}
}
